...
<script type="text/javascript">
	function dosave(dialog,grid) {
		if ($(".form").form('validate')) {
			$.post(ext.contextPath + "/material/salesorderproduct/update.do", $(".form").serialize(), function(data) {
				if (data.res == 1) {
...
<body>
		...
		<input id="status" name="status" type="hidden" value="${salesorderproduct.status}"/>	
		<div class="easyui-panel" title="销售订单信息" style="padding:10px;"
		data-options="tools:'#tt'">
			<table class="table">
				<tr>
					<th>销售订单编号</th>
					<td><input id="salesorderno" name="salesorderno" class="easyui-textbox" data-options="required:true,validType:'isBlank'" value="${salesorderproduct.salesorderno}" /></td>
				</tr>
				<tr>
					<th>产品编码</th>
					<td>
						<input id="productcode" name="productcode" class="easyui-textbox" data-options="required:true,validType:'isBlank'" value="${salesorderproduct.product.materialcode}" />
						<input id="productid" name="productid" type="hidden"  value="${salesorderproduct.productid}" />
					</td>
				</tr>
				<tr>
					<th>产品名称</th>
...
<!-- 						<input id="productnum" name="productnum" class="easyui-textbox" value="" /> -->
						<input id="productnum" name="productnum" class="easyui-numberspinner" data-options="min:1,max:999,required:true,validType:'isBlank',prompt:'1-999'" style="width:130px;" value="${salesorderproduct.productnum}"></input>
<!-- 						<a href="#" class="easyui-linkbutton" onclick="createDetails();">生成序列号</a> -->
				...
				<tr>
					<th>所属客户</th>
					<td>
						<input id="clientsid" name="clientsid" class="easyui-combobox" value="${salesorderproduct.clients.name}" />
					</td>
				...
				<tr>
					<th>订单创建时间</th>
					<td>
						<input id="ordercreatedate" name="ordercreatedate" class="Wdate" value="${fn:substring(salesorderproduct.ordercreatedate,0,16)}"
				...
				<tr>
					<th>订单完成时间</th>
					<td>
						<input id="orderfinishdate" name="orderfinishdate" class="Wdate" value="${fn:substring(salesorderproduct.orderfinishdate,0,16)}"
							onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})"
							readonly>
					</td>
				</tr>
				<tr>
					<th>发货时间</th>
					<td>
						<input id="deliverydate" name="deliverydate" class="Wdate" value="${fn:substring(salesorderproduct.deliverydate,0,16)}"
							onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})"
							readonly>
					</td>
				</tr>
		...
		<div class="easyui-panel" title="产品明细">
			<table id="detailtable"  class="table">
				...
				<c:set var="rowcount" value="${1}" />
					<c:forEach items="${salesorderproduct.orderproductdetail }" var="obj">
					<tr style="height:22px">
						<td style='text-align:center'>${rowcount}</td>
						<td>${obj.productuid }
							<input type="hidden" id='product${rowcount}' name='product${rowcount}' value='${obj.productuid }'  readonly/>
						</td>
					</tr>
