        ...
        _text.click(function(){
            if (!designer.config.editable) {
            	var dialog = parent.ext.modalDialog({
    				title : designer.config.title+_o.props['displayName'].value,
    				url : designer.config.url + "&taskname="+_o.props['name'].value,
    				width : designer.config.modalDialogWidth,
    				height : designer.config.modalDialogHeight,
    				onClose:function (){
    					if(dialog.find('iframe').get(0).contentWindow.returnVal != ""){
    						returnVal=dialog.find('iframe').get(0).contentWindow.returnVal;
    					}
    				}
    			});
            }
		...
		if(designer.config.editable){
			...
			snaker("#save").click(function(){
				...
				for(var node in _states){
					if(_states[node]&&_states[node].toBeforeXml()!=null){
