...
(function(snaker){
	...
	designer.rect=function(o, r){
        var _this = this, _uid = 'rect' + designer.util.nextId(),
        	_o = snaker.extend(true, {},designer.config.rect, o),
        	_id = _uid, _r = r, // Raphael画笔
            _rect, _img, // 图标
            _name, // 状态名称
            _text, // 显示文本
            _ox, _oy; // 拖动时，保存起点位置;
        ...
        _rect.dblclick(function(){
            if (designer.config.editable) {
                var returnValue = window.showModalDialog(designer.config.ctxPath,window,'dialogWidth:1000px;dialogHeight:600px');
                if(returnValue) {
                    var formPath = designer.config.formPath + returnValue + ".html";
                    _o.props.form.value = formPath;
                    document.getElementById("pform").innerHTML = '<input style="width:100%;" value="' + formPath + '"/>';
                }
            }
        });
            ...
            for(var k in _o.props){
                if(k=="name"&&_o.props[k].value==""){
                	if(_o.type=="start"){
                		...
                		alert("工序名称不能为空");
                	}else if(_o.type=="custom"){
                		alert("自定义任务名称 不能为空");
                	}else if(_o.type=="subprocess"){
                		alert("子流程名称 不能为空");
                	}else if(_o.type=="decision"){
                		alert("决策点名称 不能为空");
                	}else if(_o.type=="fork"){
                		alert("分支点名称 不能为空");
                	}else if(_o.type=="join"){
                		alert("汇合点名称 不能为空");
                	}else{
	...
	designer.editors={
		textEditor:function(){
			...
			this.init=function(props, _k, _div, _src, _r){
				...
				snaker('<input style="width:100%;"/>').val(_src.text()).change(function(){
	...
	designer.init=function(c, o){
		...
		if(designer.config.editable){
			//设置默认选择状态
			snaker("#pointer").addClass("selected");
			...
			snaker("#save").click(function(){
				...
				for(var node in _states){
					if(_states[node]){
						//判断是否重名name为用户定义任务名称，id为raphael命名的图形名
						if(_states[node].getName()){
							if(hash[_states[node].getName()]){
								alert(_states[node].getName()+" 名称已存在，请修改！");
								return;
							}else{
								hash[_states[node].getName()] = true;
							}
						}
						
						//保证节点非空
						if(_states[node].toBeforeXml()){
							temp+=_states[node].toBeforeXml();
